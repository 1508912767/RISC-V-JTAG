...
#define CLEAR_CMDERR_ERROR 6
	...
	// sv export dpi to implement
	extern void read_dm_register(int reg_num,  uint32_t* value, int* error);
	extern void write_dm_register(int reg_num, uint32_t reg_value, int* error);
	extern void debug_error();
    extern void debug_finish();
	extern void delay_us();

/*
	extern void access_dtm_register(int reg_num, int tdi_size, uint32_t tdi_data, uint32_t* tdo_data);

	// jtag operation
	extern void jtag_reset();   // reset jtag and jtag enter idle state
	extern void jtag_idle_to_shift_ir();

	// jtag in send_ir state, send the ir data, then jtag enter exit state
	extern void jtag_send_ir(int ir_reg);
	extern void jtag_idle_to_shift_dr();
	// jtag in send_dr state, send the dr data, then jtag enter exit state
	extern void jtag_send_dr(uint64_t tdi_reg, uint64_t *tdo_reg, int size);
	extern void jtag_exit_to_idle();
	extern void jtag_keep_in_idle(int number);
	
	extern int get_jtag_wire_mode();
	extern void get_simulate_time(uint64_t* simtime);
*/
...
int enable_dmi(); // enable dmi, debugger can access dm

int halt_cpu(); // halt cpu
int resume_cpu(); // result cpu
...
// 0: no error
// 1: access error
int read_csr(int reg_num, uint64_t * value, ACCESS_MODE mode); //ABS_CMD
int write_csr(int reg_num, uint64_t value, ACCESS_MODE mode);  //ABS_CMD
...
// 0: no error
// 1: access error
int read_memory(uint64_t addr , uint64_t *data, MEM_ACCESS_SIZE size, ACCESS_MODE mode); //ABS_CMD
int write_memory(uint64_t addr , uint64_t data, MEM_ACCESS_SIZE size, ACCESS_MODE mode); //ABS_CMD

int write_memory_bulk_ABS_CMD(uint64_t base_addr, uint64_t *data_array, int len, MEM_ACCESS_SIZE size); //MEM_SIZE_32
//int read_memory_bulk_ABS_CMD(uint64_t base_addr, uint64_t *data_array, int len, MEM_ACCESS_SIZE size = MEM_SIZE_32);
